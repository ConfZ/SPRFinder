...
from SMT_generator.scanner import scan
from SMT_generator.ast import *
from SMT_generator.util import join_terms_with
import SMT_generator.var_pool as var_pool
...
def accept_arg(s):
    ...
    if s.accept('IDENTIFIER'):
        ...
        # print(var_pool.const_str_var)
        if var_name in var_pool.int_var:
            return VarNode(var_name, 'Int')
        elif var_name in var_pool.str_var:
            return VarNode(var_name, 'String')
        elif var_name in var_pool.const_int_var:
            return ConstVarNode(var_name, 'Int')
        elif var_name in var_pool.const_str_var:
            return ConstVarNode(var_name, 'String')
        else:
...
def expect_expression(s):
    ...
    if s.accept('DECLARE_FUN'):
        ...
        var_sort = return_sort.name
        if var_sort == 'Int':
            var_pool.int_var.append(var_name)
        elif var_sort == 'String':
            var_pool.str_var.append(var_name)
        return FunctionDeclarationNode(VarNode(var_name, var_sort), BracketsNode(signature), return_sort)
    ...
    if s.accept('DECLARE_CONST'):
        name        = expect_identifier(s)
        return_sort = expect_sort(s)
        if return_sort.name == 'Int':
            var_pool.const_int_var.append(name.name)
        elif return_sort.name == 'String':
            var_pool.const_str_var.append(name.name)
        # print(var_pool.const_str_var)
        return ConstantDeclarationNode(ConstVarNode(name.name, return_sort.name), return_sort)
...
def get_expressions(s):
    var_pool.int_var = []
    var_pool.str_var = []
    var_pool.const_int_var = []
    var_pool.const_str_var = []
    expressions = []
    s.advance()
...
def parse_file(path, language=SMT_25_STRING):
