...
from SMT_generator.smt import smt_new_var, smt_reset_counters, smt_declare_var, smt_len, smt_assert, smt_equal
...
def make_random_expression(variables, sort, depth, valid_override=False):
    ...
    random_args = [make_random_expression(variables, arg_sort, shrunken_depth, valid_override) for arg_sort in
                   signature]
...
def make_random_ast(num_vars, int_num, bool_num, num_asserts, depth, max_terms, max_str_lit_length, max_int_lit,
                    literal_probability, semantically_valid):
    global _max_terms
    ...
    # create variables
    variables = {}
    str_variables = {STRING_SORT: [smt_new_var() for i in range(num_vars)]}
    int_variables = {INT_SORT: [smt_new_var() for i in range(int_num)]}
    bool_variables = {BOOL_SORT: [smt_new_var() for i in range(bool_num)]}
    variables.update(str_variables)
    variables.update(int_variables)
    variables.update(bool_variables)
    # create declarations
    declarations = []
    ...
    strlen = random.randint(0, 3)
    if strlen != 0:
        svars = random.choices(variables[STRING_SORT], k=strlen)
    else:
        svars = []
    for v in svars:
        asserts = asserts + [smt_assert(smt_equal(v, make_random_literal(STRING_SORT)))]
    # add check-sat
