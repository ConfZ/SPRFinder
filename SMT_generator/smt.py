...
# helper functions
def smt_init():
    global var_counter, const_counter
    var_counter = 0
    const_counter = 0
...
def new_var():
    ...
    returned = var_counter
    var_counter += 1
    return VAR_PREFIX + str(returned)
def smt_new_const():
...
def smt_substring(var1, int1, int2):
    return SubstringNode(var1, int1, int2)
...
def smt_suffixof(a, b):
    return SuffixOfNode(b, a)


def smt_prefixof(a, b):
    return PrefixOfNode(b, a)
...
def smt_string_arithmatic_logic():
