...
from SMT_generator.smt import new_var, smt_reset_counters, smt_declare_var
...
def make_random_ast(num_vars, num_asserts, depth, max_terms, max_str_lit_length, max_int_lit, literal_probability,
                    semantically_valid):
    global _max_terms
    ...
    # create variables
    variables = [VarNode(new_var(), s) for i in range(num_vars) for s in DECLARABLE_SORTS]
