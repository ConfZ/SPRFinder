...
class VarNode(IdentifierNode):
    def __init__(self, var_name, var_sort):
        super().__init__(var_name)
        self.sort = var_sort
    def __repr__(self):
        return '\'{}Var<{}>\''.format(self.sort, self.name)
class ConstVarNode(IdentifierNode):
    def __init__(self, var_name, var_sort):
        super().__init__(var_name)
        self.sort = var_sort
    def __repr__(self):
        return '\'Const{}Var<{}>\''.format(self.sort, self.name)
...
class LiteralNode(_SortedASTNode):
    ...
    def __repr__(self):
        return '\'{}<{}>\''.format(self.get_sort(), re.sub(r' ','&nbsp;',str(self.value)))
...
# math node
class MathNode(_BinaryExpression):
    _signature = [INT_SORT, ANY_SORT]
    _sort      = INT_SORT
...
class MutiNode(MathNode):
    _symbol    = '*'

class DivNode(MathNode):
    _symbol    = 'Div'
# relations
class MathRelationNode(_RelationExpressionNode):
    _signature = [INT_SORT, INT_SORT]
    def __index__(self, symbol, a, b):
        super.__init__(a, b)
        _symbol    = symbol
class EqualNode(_BinaryExpression):
    _signature = [ANY_SORT, ANY_SORT]
    _sort      = BOOL_SORT
    _symbol    = '='

class GtNode(MathRelationNode):
    _symbol = '>'

class LtNode(MathRelationNode):
    _symbol = '<'

class GteNode(MathRelationNode):
    _symbol = '>='

class LteNode(MathRelationNode):
    _symbol = '<='
...
class IndexOf2Node(_TernaryExpression):
    ...
    _sort      = INT_SORT
    _symbol    = 'IndexOf2'
class IteNode(_TernaryExpression):
    _signature = [STRING_SORT, STRING_SORT, INT_SORT]
    _sort      = BOOL_SORT
    _symbol    = 'Ite'
...
class IntPlusNode(_BinaryExpression):
    _signature = [INT_SORT, INT_SORT]
    _sort = INT_SORT
    _symbol = '+'
class IntMinusNode(_BinaryExpression):
    _signature = [INT_SORT, INT_SORT]
    _sort = INT_SORT
    _symbol = '-'
