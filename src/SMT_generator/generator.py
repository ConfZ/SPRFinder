...
def generate_node(node, language):
    ...
    raise NotImplementedError('no generator for {}'.format(type(node)), 'Node is:', node)
...
def generate(ast, language=SMT_25_STRING):
    return '\n'.join(generate_node(e, language) for e in ast)

# def generate_declare()
